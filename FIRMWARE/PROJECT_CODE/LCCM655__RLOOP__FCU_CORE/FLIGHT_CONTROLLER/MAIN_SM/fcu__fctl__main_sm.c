...
Luint8 u8FCU_FCTL_MAINSM__CheckIfCoasting(void)
{
	...
	Luint32 u32PodSpeed = u32FCU_FCTL_NAV__PodSpeed();
	//
	if(u32PodSpeed && (u8PusherSwitch1 == 0U) && (u8PusherSwitch2 == 0U) && (eFCU_FCTL_EDDY_BRAKES__Get_State() == EDDYBRAKES_STATE__RETRACTED))
	{
		u8Test = 1U;
	}
	else
...
{
	Luint8 u8Test;
	if(eFCU_FCTL_EDDY_BRAKES__Get_State() == EDDYBRAKES_STATE__BRAKING)
	{
		u8Test = 1U;
	}
	else
...
{
	Luint8 u8Test;
	if(eFCU_FCTL_EDDY_BRAKES__Get_State() == EDDYBRAKES_STATE__CONTROLLED_BRAKING)
	{
		u8Test = 1U;
	}
	else
